...
echo ${model}
  for lr_lambda in ${lr_lambdas[*]}; do
    ...
    for lr_b in ${lr_bs[*]}; do
      nohup python ../fedDHADScaffoldmain.py --iid 0 --equal ${equal} --unequal ${unequal} --unbalance ${unbalance} --dirichlet ${dirichlet} --dataset ${dataset} \
      ...
      --model ${model} --l ${l} --optim ${optim} --lambda_decay ${lambda_decay} --seed ${seed} --control ${control}\
