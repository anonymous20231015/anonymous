...
echo ${model}
  for lr_lambda in ${lr_lambdas[*]}; do
    ...
    for lr_b in ${lr_bs[*]}; do
      nohup python ../fedDHCDEmain.py --iid 0 --equal ${equal} --unequal ${unequal} --unbalance ${unbalance} --dirichlet ${dirichlet} --dataset ${dataset} \
      --result_dir ${dir_name} --epochs ${epochs} --local_bs ${local_bs} --local_ep ${local_ep} --decay ${decay}\
      --lr_lambda ${lr_lambda}  --lr_b ${lr_b} --b_decay ${b_decay}\
