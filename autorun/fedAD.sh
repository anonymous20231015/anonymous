...
echo ${model}
  nohup python ../fedADmain.py --equal ${equal} --unequal ${unequal} --unbalance ${unbalance} --dirichlet ${dirichlet} --dataset ${dataset} \
    --result_dir ${dir_name} --epochs ${epochs} --local_bs ${local_bs} --local_ep ${local_ep} --decay ${decay}\
    --model ${model} --l ${l} --optim ${optim} --seed ${seed} --start_epoch ${start_epoch}\
