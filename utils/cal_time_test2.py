...
if method == "fedAD":
    ...
    # FjORD
    fjord_weight_file_path = os.path.join(root_path, f"result/{data_name}/{model_name}/FjORD_2_6{seed}_{distribution}{suffix}/log", f"weights_{seed}.txt")
    fjord_acc_file_path = os.path.join(root_path,
                                         f"result/{data_name}/{model_name}/FjORD_2_6{seed}_{distribution}{suffix}",
    ...
    fedavg_acc_fill_path = os.path.join(root_path, f"result/{data_name}/{model_name}/fedavg{seed}_{distribution}{suffix}",
                                        f"_test_accuracy.txt")
    ...
    DH_acc_file_path = os.path.join(root_path,
                                    ...
                                    f"_test_accuracy.txt")
    DHAD_weight_path = os.path.join(root_path,
                                    f"result/{data_name}/{model_name}/fedDHAD{seed}_{distribution}_lam-decay{DHAD_lam_decay}_b-decay{DHAD_b_decay}{suffix}/{DHAD_lam_lr}/{DHAD_b_lr}/log",
                                    f"weights_{seed}.txt")
    DHAD_acc_path = os.path.join(root_path,
                                 f"result/{data_name}/{model_name}/fedDHAD{seed}_{distribution}_lam-decay{DHAD_lam_decay}_b-decay{DHAD_b_decay}{suffix}/{DHAD_lam_lr}/{DHAD_b_lr}",
                                 f"_test_accuracy.txt")
    try:
        with open(fedavg_acc_fill_path, "r") as f:
            fedavg_acc = eval(f.read())
...
print("base_flop: {}, base_size: {}".format(base_flop, base_size))
...
print("training time: {}, comm_time: {}".format(training_time, comm_time))
print("epoch time = training_time * flop / base_flop + comm_time * size / base_size")
...
def test():
    if method=="fedAD":
        ...
        if zoom == 0:
            ...
            a = plt.plot([i / 1000 for i in fedavg_time[:location]], fedavg[:location], label='FedAvg')
            b = plt.plot([i / 1000 for i in fedDrop_time[:location]], fedDrop[:location], label='FedDrop')
            c = plt.plot([i / 1000 for i in afd_time[:location]], AFD[:location], label='AFD')
            d = plt.plot([i / 1000 for i in AD_time[:location]], fedAD[:location], label='FedAD')
            e = plt.plot([i / 1000 for i in AD_time[:location]], DHAD[:location], label='FedDHAD')
            plt.tick_params(labelsize=14)
            plt.legend(fontsize=mode["legend"])
            plt.xlabel("Training time (10$^{3}$ s)", fontsize=mode["label"])
            plt.ylabel("Accuracy", fontsize=mode["label"])
            ...
            # 嵌套
            left, bottom, width, height = 0.4, 0.3, 0.25, 0.25
            ...
            for i in DHAD:
                if i >= acc_tar:
                    ...
                    break
            if y_fedavg > 0:
                ax2.plot([i / 1000 for i in fedavg_time[y_fedavg:location]], fedavg[y_fedavg:location],
                         color=a[0].get_color())
            if y_feddrop > 0:
                ax2.plot([i / 1000 for i in fedDrop_time[y_feddrop:location]], fedDrop[y_feddrop:location],
                         color=b[0].get_color())
            if y_afd > 0:
                ax2.plot([i / 1000 for i in afd_time[y_afd:location]], AFD[y_afd:location], color=c[0].get_color())
            if y_fedAD > 0:
                ax2.plot([i / 1000 for i in AD_time[y_fedAD:location]], fedAD[y_fedAD:location], color=d[0].get_color())
            if y_fedDHAD > 0:
                ax2.plot([i / 1000 for i in AD_time[y_fedDHAD:location]], DHAD[y_fedDHAD:location],
                         color=e[0].get_color())
        if pdf:
            plt.savefig(f'FedAD_{data_name}_{model_name}.pdf')
        else:
            plt.show()
    else:
        ...
        fedavg_time, DHAD_time, total_fedAvg, total_fedDHAD = cal_MFLOPs_and_time(method)
        ...
        for i in DHAD:
            if i >= target_acc:
                y_fedDHAD = DHAD.index(i)
                break
        print("到达目标精度：{} FedDHAD所用时间：{}".format(target_acc, DHAD_time[y_fedDHAD]))

        print("FedAvg MFLOPs:", total_fedAvg / (epoch*10*10**6))
        print("FedDHAD MFLOPs:", total_fedDHAD / (epoch*10*10**6))
        ...
        # print("相比FedAvg时间提升了：", (fedavg_time[:location][-1] - DHAD_time[:location][-1]) / fedavg_time[:location][-1])
        plt.figure(figsize=[mode["fig"][0], mode["fig"][1]])
        plt.plot([i / 1000 for i in fedavg_time[:location]], fedavg[:location], label='FedAvg')
