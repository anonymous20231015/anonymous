...
from flearn.utils.model_utils import is_conv, is_fc
...
def hrank_prune(model, feature_result_list,prune_rate,layer_idx=0, device="cpu", baselinename=None):
    """
    ...
    feature_result_list = [torch.tensor(0.)] * len(model.relu_layers)
    total_list = [torch.tensor(0.)] * len(model.relu_layers)
     将特征图秩保存在 feature_result_list 中
    get_rank(model, feature_result_list, total_list, train_loader, layer_idx)
    print(feature_result_list)
    prune_symbol = get_drop_rate()'''
    _, ind = model.unstructured_by_rank(feature_result_list, prune_rate, layer_idx, device,baselinename=baselinename)
    return ind

def get_all_feature(model, dataset, idxs, layer_idx=0):
    train_loader = DataLoader(DatasetSplit(dataset, idxs), batch_size=100, shuffle=True)
    feature_result_list = [torch.tensor(0.)] * len(model.relu_layers)
    total_list = [torch.tensor(0.)] * len(model.relu_layers)
    # print(len(model.relu_layers))
    # 将特征图秩保存在 feature_result_list 中
    get_rank(model, feature_result_list, total_list, train_loader, layer_idx)
    return feature_result_list
def get_weight(model, layer_index):
    layer = model.prunable_layers[layer_index]
    nearon_nums = len(layer.weight)
    layer_weight = torch.zeros(nearon_nums)
    if layer_index < len(model.prunable_layers)-1 and is_fc(model.prunable_layers[layer_index + 1]):
        for i in range(nearon_nums):
            layer_weight[i] = torch.sum(torch.abs(layer.weight[i])) + torch.sum(torch.abs(model.prunable_layers[layer_index+1].weight), dim=0)[i]
    if layer_index == len(model.prunable_layers)-1 or not is_fc(model.prunable_layers[layer_index+1]):
        for i in range(nearon_nums):
            layer_weight[i] = torch.sum(torch.abs(model.prunable_layers[layer_index].weight[i]))
    return layer_weight
def make_new_mask(model, prune_ratio, device):
    ...
    new_mask = {k: torch.LongTensor(v).to(device)
                for k, v in new_mask.items()}
    inv_mask = {k: torch.LongTensor(v).to(device)
...
def get_rate_for_each_layers(model, global_prune_rate):
    """
    根据模型权重的范式以及全局剪枝率，为每一层卷积层和全连接层确定剪枝率
    :param model:
    :param global_prune_rate:
    :return:
    """
    feature_weights = {k: v.clone().cpu().detach().numpy()
               for k, v in model.features.named_parameters()
               if v.requires_grad}
    classifier_weights = {k: v.clone().cpu().detach().numpy()
                       for k, v in model.classifier.named_parameters()
                       if v.requires_grad}
    # flat the weights
    feature_weight_flat = np.concatenate([v.flatten() for k, v in feature_weights.items()])
    classifier_weight_flat = np.concatenate([v.flatten() for k, v in classifier_weights.items()])

    # get the thredsheld
    number_of_feature_weights_to_prune = int(np.ceil(global_prune_rate * feature_weight_flat.shape[0]))
    feature_threshold = np.sort(np.abs(feature_weight_flat))[number_of_feature_weights_to_prune]
    number_of_classifier_weights_to_prune = int(np.ceil(global_prune_rate * classifier_weight_flat.shape[0]))
    classifier_threshold = np.sort(np.abs(classifier_weight_flat))[number_of_classifier_weights_to_prune]
    compress_rate = []
    for i, layer in enumerate(model.prunable_layers):
        if is_conv(layer):
            a = np.abs(layer.weight.data.clone().cpu().detach().numpy()) < feature_threshold
            b = int(a.sum())
            c = layer.weight.data.numel()
            compress_rate.append(round(b / c, 4))
        if is_fc(layer):
            a = np.abs(layer.weight.data.clone().cpu().detach().numpy()) < classifier_threshold
            b = int(a.sum())
            c = layer.weight.data.numel()
            compress_rate.append(round(b / c, 4))
...
def lt_mask_copy(model, mask):
    ...
    for k, v in mask.items():
        ...
        if k.endswith("weight"):
            layer.mask = v.clone().to(layer.mask.device) * layer.mask

        if k.endswith("bias"):
            layer.bias_mask = v.clone().to(layer.mask.device) * layer.bias_mask
...
def get_ratio(model, dataset, idxs, device="cpu"):
    """
    根据特征值的概率密度，获取剪枝率
    :param model:
    :param dataset:
...
if __name__=="__main__":
    ...
    model = cnn.CNN()
    # model = VGG11()
    get_ratio(model, train_dataset, user_groups[100], "cpu")
