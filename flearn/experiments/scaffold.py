...
import sys
# sys.path.append('/home/mabeichen/FedHAVG/')
sys.path.append("/Users/mabeichen/PycharmProjects/FedHAVG")
import numpy as np

from data.util import get_global_distribution, get_client_noniid_degree
from flearn.utils.model_utils import test_inference, average_weights
...
class Scaffold(object):
    ...
    """
    def __init__(self, args, iid, equal=True, unequal=False, unbalance=False, dirichlet=True, l=2, result_dir="fedavg"):
    ...
    def init_data(self):
        if self.args.dataset == "cifar10":
            ...
            train_dataset, test_dataset, user_groups = get_dataset(num_data=self.num_data,
                                                                   num_users=self.args.num_users, equal=self.equal,
                                                                   unequal=self.unequal, unbalance=self.unbalance,
                                                                   dirichlet=self.dirichlet, l=self.l)
    ...
    def load_model(self):
        # BUILD MODEL
        if self.args.dataset == "cifar10" or "svhn":
            in_channel = 3
            num_classes = 10
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            start = time.time()
            local_weights, local_losses, local_delta = [], [], []
            ...
            local_delta = self.client_train(idxs_users, global_model, user_groups, epoch, train_dataset, train_losses, local_weights, local_losses, local_delta)
...
if __name__ == '__main__':
    args = args_parser()
    t = Scaffold(args, iid=False, equal=False, unequal=False, unbalance=False, dirichlet=True)
    t.train()
