...
from data.util import get_global_distribution, get_client_noniid_degree, get_target_users_distribution, \
    get_noniid_degree, get_user_distribution
...
class FedHACDE(object):
    ...
    def __init__(self, args, iid, equal=True, unequal=False, unbalance=False, dirichlet=False, l=2, result_dir="fedhavg"):
        ...
        self.thetas = [[(float(0.0)) for j in range(self.args.num_classes)] for i in range(self.args.num_users)]
        self.JS_divergence = [(float(0.0)) for _ in range(self.args.num_users)]
    ...
    def get_class_distribution(self, user_groups, train_dataset, upload_model, idx_user):
        """
        获取验证数据的loss gradient
        :param user_groups:
        :param train_dataset:
        :param global_model:
        :return:
        """
        local_model = LocalUpdate(args=self.args, local_bs=32, dataset=train_dataset,
                                  idxs=user_groups[self.args.num_users], device=self.device)
        squared_loss_grads = local_model.calculate_loss_grads(copy.deepcopy(upload_model))
        # print(idx_user, "grads:", squared_loss_grads)

        squared_loss_grads = np.array(squared_loss_grads)
        # squared_loss_grads = squared_loss_grads.min() / squared_loss_grads

        def softmax(grad_sum, temp=1):

            # grad_sum = grad_sum - grad_sum.mean()
            D = np.max(grad_sum)
            # return np.exp(temp / grad_sum) / np.exp(temp / grad_sum).sum()
            return np.exp( (grad_sum / temp) - D) / np.exp( (grad_sum / temp) - D).sum()

        theta = softmax(squared_loss_grads, temp=1)
        theta = theta / theta.sum()
        # print(idx_user, "theta:", theta)

        # self.thetas[idx_user] = [theta[i] - self.thetas[idx_user][i] for i in range(len(theta))]
        self.thetas[idx_user] = theta
        p_k = [math.ceil(a * self.args.local_ep * len(user_groups[idx_user])) for a in self.thetas[idx_user]]
        return p_k
    ...
    def niid_aggregation(self, w, idx):
        ...
        for i in range(0, len(w)):
            print("client {} | Non-iid degree: {} | lambda {}".format(idx[i], self.JS_divergence[idx[i]], self.lambdas[idx[i]]))
            total += (w[i][0] / ((self.lambdas[idx[i]] * self.JS_divergence[idx[i]]) + self.b[idx[i]]))
        for key in w_avg.keys():
            # print("the size of w: ", w_avg[key].size())
            w_avg[key] *= (w[0][0] / ((self.lambdas[idx[0]] * self.JS_divergence[idx[0]]) + self.b[idx[0]]))
            for i in range(1, len(w)):
                w_avg[key] += (w[i][1][key] * (w[i][0] / ((self.lambdas[idx[i]] * self.JS_divergence[idx[i]]) + self.b[idx[i]])))
    ...
    def b_update(self, lr, local_weights, idxs_users, local_grads):
        ...
        for n, k in enumerate(idxs_users):
            ...
            for i in range(0, len(local_weights)):
                if idxs_users[i] != k:
                    a += (local_weights[i][0] / (self.lambdas[idxs_users[i]] * self.JS_divergence[idxs_users[i]] + self.b[idxs_users[i]]))
                else:
                    k_index = i
            total = pow(a + (local_weights[k_index][0] / (self.lambdas[k] * self.JS_divergence[k] + self.b[k])), 2) * pow(self.b[k] + self.lambdas[k] * self.JS_divergence[k], 2)
                ...
                for i in range(0, len(local_weights)):
                    if idxs_users[i] != k:
                        w[key] += (local_weights[i][1][key] * local_weights[k_index][0] * (local_weights[i][0] / (
                                self.lambdas[idxs_users[i]] * self.JS_divergence[idxs_users[i]] + self.b[idxs_users[i]])))
                    w[key] = torch.div(w[key], total)
        ...
        for n, k in enumerate(idxs_users):
            ...
            for i in range(0, len(local_weights)):
                if idxs_users[i] != k:
                    ...
                    k_index = i
            total = pow(a + (local_weights[k_index][0] / (self.lambdas[k] * self.JS_divergence[k] + self.b[k])), 2) * pow(
                self.b[k] + self.lambdas[k] * self.JS_divergence[k], 2)

            w = copy.deepcopy(local_weights[k_index][1])
            for key in w.keys():
                w[key] *= a * (-local_weights[k_index][0]) * self.JS_divergence[k_index]
                w[key] = torch.div(w[key], total)

            for key in w.keys():
                for i in range(0, len(local_weights)):
                    if idxs_users[i] != k:
                        w[key] += (local_weights[i][1][key] * local_weights[k_index][0] * self.JS_divergence[k_index] * (local_weights[i][0] / (
                                self.lambdas[idxs_users[i]] * self.JS_divergence[idxs_users[i]] + self.b[idxs_users[i]])))
                    w[key] = torch.div(w[key], total)
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            g_model = copy.deepcopy(global_model.state_dict())
            for i in range(len(idxs_users)):
                if sum(self.thetas[idxs_users[i]]) == 0:
                    global_model.load_state_dict(local_weights[i][1])
                    p_k = self.get_class_distribution(user_groups=user_groups, train_dataset=train_dataset, upload_model=global_model, idx_user=idxs_users[i])
                    client_JS_divergence = get_noniid_degree(p_k, self.global_distribution)
                    print("client {} JS: {:4f} | distribution: {}".format(idxs_users[i], client_JS_divergence, p_k), end="  |  ")
                    if (i + 1) % 2 == 0:
                        print()
                    self.JS_divergence[idxs_users[i]] = client_JS_divergence
            print()

            global_model.load_state_dict(g_model)
...
if __name__ == '__main__':
    args = args_parser()
    t = FedHACDE(args, iid=False, equal=False, unequal=False, unbalance=False, dirichlet=True)
    t.train()
