...
class HRank(CentralTraining):
    ...
    def load_balance(self, dropout_rates, user_groups, max_device_dprate, max_device):
        P = max_device_dprate
        for i in range(len(dropout_rates)):
            rate = 1 - (len(user_groups[max_device]) * (1 - P) / len(user_groups[i]))
            if rate <= 0:
    ...
    def train(self):
        ...
        for i in range(100):
            if sample_nums < len(user_groups[i]):
                sample_nums = len(user_groups[i])
                max_device = i
        ...
        for epoch in range(self.args.epochs):
            ...
            if (epoch > 0 and epoch % dropout_step == 0) or self.find_first_min_diff:
                for loc, idx in enumerate(self.sorted_conv_layers):
                    feature_result = get_all_feature(global_model, train_dataset, user_groups[max_device], idx)
                ...
                if len(record_rank) > 0:
                    ...
                    if cur_diff < self.pre_diff and self.find_first_min_diff is False:
                        self.pre_diff = cur_diff
                        self.pre_model = copy.deepcopy(global_model)
                    elif cur_diff >= self.pre_diff and self.find_first_min_diff is False and epoch >= self.args.start_epoch:
                        global_model = copy.deepcopy(self.pre_model)
                        self.max_device_ret = get_ratio(global_model, train_dataset, user_groups[max_device], self.device)
                        update_ratio_rank_list = feature_result_list
                        self.load_balance(self.dropout_rates, user_groups, self.max_device_ret,max_device)
                        self.find_first_min_diff = True
                        self.start_dropout = True
                        self.pre_diff = cur_diff
                        self.get_dropout_tatios.append([epoch, self.pre_diff, self.dropout_rates])
                    elif self.find_first_min_diff and cur_diff < self.pre_diff and epoch <= self.args.epochs / 2:
                        max_device_ret = get_ratio(global_model, train_dataset, user_groups[max_device], self.device)
                        if max_device_ret < self.max_device_ret:
                            self.max_device_ret = max_device_ret
                            self.load_balance(self.dropout_rates,user_groups, self.max_device_ret, max_device)
                            update_ratio_rank_list = feature_result_list
                            self.get_dropout_tatios.append([epoch, self.pre_diff, self.dropout_rates])
                            self.pre_diff = cur_diff
