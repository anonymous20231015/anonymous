...
from flearn.utils.prune import hrank_prune, get_ratio, get_all_feature,get_weight
...
class FedDHADScaffold(object):
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            local_losses, local_grads, local_delta = self.client_train(idxs_users, users_model, user_groups, epoch, train_dataset, train_losses, local_weights, local_losses, local_grads, local_delta)
        ...
        avg_rank_weight_path = os.path.join(result_path, str(self.args.lr_b), "log",
                                            f"avg_rank_weight_{self.args.seed}.txt")
...
if __name__ == '__main__':
    args = args_parser()
    t = FedDHADScaffold(args, iid=False, equal=False, unequal=False, unbalance=False, dirichlet=True)
    t.train()
