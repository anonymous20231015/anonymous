            ...
            if (epoch > 0 and epoch % dropout_step == 0) and self.start_dropout is False:
                ...
                for device in p_devices:
                    ...
                    for i in range(len(feature_result_list)):
                        mean_feature_result_list[i] = torch.add(mean_feature_result_list[i], feature_result_list[i])
                for i in range(len(feature_result_list)):
                    mean_feature_result_list[i] = torch.div(mean_feature_result_list[i], len(p_devices))
                ...
                if len(record_rank) > 0:
                    ...
                    if cur_diff < self.pre_diff:
                        ...
                        for device in p_devices:
                            ...
                            mean_ratio.append(device_ret)
                        device_ratio = np.mean(mean_ratio)
