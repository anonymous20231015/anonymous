...
class FedHA(object):
    ...
    """
    def __init__(self, args, iid, equal=True, unequal=False, unbalance=False, update_lambda=True,
                 regular=True, l=2, result_dir="fedhavg", client_mu=0.0):
    ...
    def init_data(self):
        if self.args.dataset == "cifar10":
            ...
            train_dataset, test_dataset, user_groups = get_dataset(num_data=self.num_data,
                                                    num_users=self.args.num_users, equal=self.equal, unequal=self.unequal, unbalance=self.unbalance, l=self.l)
    ...
    def client_train(self, idx_users, global_model, user_groups, epoch, train_dataset, train_losses, local_weights,
        ...
        for idx in idx_users:
            ...
            if self.args.control:
                w, loss, grad, cnt = local_model.update_weights(model=copy.deepcopy(global_model), global_round=epoch,
                                                          mu=self.args.client_mu, c=self.server_c,
                                                          ci=self.clients_c[idx])
    ...
    def niid_aggregation(self, w, idx):
        ...
        for i in range(0, len(w)):
            ...
            total += (w[i][0] / (self.lambdas[idx[i]] * self.niid_degree[idx[i]]))
        for key in w_avg.keys():
            # print("the size of w: ", w_avg[key].size())
            w_avg[key] *= (w[0][0] / (self.lambdas[idx[i]] * self.niid_degree[idx[0]]))
            for i in range(1, len(w)):
                w_avg[key] += (w[i][1][key] * (w[i][0] / (self.lambdas[idx[i]] * self.niid_degree[idx[i]])))
            w_avg[key] = torch.div(w_avg[key], total)

        return w_avg

    def lambda_update(self, lr, local_weights, idxs_users, local_grads):
        # sever端 update lambda
        result = []
        # for k in C * K:
        for n, k in enumerate(idxs_users):
            # print("更新设备 {} lambda {}".format(k, self.lambdas[k]))
            # calculate lambda grad of F
            a = 0
            k_index = 0
            for i in range(0, len(local_weights)):
                if idxs_users[i] != k:
                    a += (local_weights[i][0] / (self.lambdas[idxs_users[i]] * self.niid_degree[idxs_users[i]]))
                else:
                    k_index = i
            total = (pow(a + (local_weights[k_index][0] / (self.lambdas[k] * self.niid_degree[k])), 2) * pow(
                self.lambdas[k], 2) * self.niid_degree[k])
                ...
                for i in range(0, len(local_weights)):
                    if idxs_users[i] != k:
                        w[key] += (local_weights[i][1][key] * local_weights[k_index][0] * (local_weights[i][0] / (
                                self.lambdas[idxs_users[i]] * self.niid_degree[idxs_users[i]])))
                    w[key] = torch.div(w[key], total)
            ...
            for i in range(len(local_grads)):
                sum_result = []
                # print("lamba {} grad * grad f({})".format(k, idxs_users[i]))
                for key, grad in zip(w.keys(), local_grads[i]):
                    # w[key] = w[key].to("cpu")
                    # grad = grad.to("cpu")
                    inner_res = torch.tensordot(w[key], grad, dims=([-1], [-1]))
                    # inner_res = torch.inner(w[key], grad)
                    # print(inner_res.size())
                    sum_res = torch.sum(inner_res)
                    # print(sum_res == tes)
                    # print(sum_res)
                    sum_result.append(sum_res)
                # print("lambda {} grad * grad f({}) = {}".format(k, idxs_users[i], sum(sum_result)))
                grad_inner_result += sum(sum_result) * local_weights[i][0]
                n_total += local_weights[i][0]
                # print("SUM += inner result * num_data: {} * {} = {}".format(sum(sum_result), local_weights[i][0], grad_inner_result))
            r = torch.div(grad_inner_result, n_total)
            # print(r)
            t = time.time()
            print(t-start)
            # Regularization (self.lambdas[k])
            result.append(torch.add(r, self.lambdas[k]))
            # print("Regularization : {} + {}".format(r, self.lambdas[k]))
    ...
    def lambda_update_noRegular(self, lr, local_weights, idxs_users, local_grads):
        # sever端 update lambda
        result = []
        # for k in C * K:
        for n, k in enumerate(idxs_users):
            ...
            for i in range(0, len(local_weights)):
                if idxs_users[i] != k:
                    # print("j: ", idxs_users[i])
                    # print("n_j: {} ".format(local_weights[i][0]))
                    # print("lambda {} | niid degree {}".format(self.lambdas[idxs_users[i]], self.niid_degree[idxs_users[i]]))
                    a += (local_weights[i][0] / (self.lambdas[idxs_users[i]] * self.niid_degree[idxs_users[i]]))
                else:
                    k_index = i
            total = (pow(a + (local_weights[k_index][0] / (self.lambdas[k] * self.niid_degree[k])), 2) * pow(
            ...
            # print("lambda grad")
            # for key in w.keys():
            #     print("{} | lambda grad {}".format(key, w[key]))

            # loss grad
            # print("w grad")
            # for grad in local_grads[n]:
            #     print(grad)
            ...
            for i in range(len(local_grads)):
                ...
                for key, grad in zip(w.keys(), local_grads[i]):
                    ...
                    # print(sum_res)
                    sum_result.append(sum_res)
                # print("lamba {} grad * grad f({}) = {}".format(k, idxs_users[i], sum(sum_result)))
                grad_inner_result += sum(sum_result) * local_weights[i][0]
                n_total += local_weights[i][0]
                # print("SUM += inner result * num_data: {} * {} = {}".format(sum(sum_result), local_weights[i][0], grad_inner_result))
            r = torch.div(grad_inner_result, n_total)
        ...
        #     sum_results = []
        #     for key, grad in zip(w.keys(), local_grads[n]):
        #         # print("lambda grad {} | w grad {}".format(w[key].size(), grad.size()))
        #         inner_res = torch.inner(w[key], grad)
        #         # print(inner_res.size())
        #         sum_res = torch.sum(inner_res)
        #         # print(sum_res)
        #         sum_results.append(sum_res)
        #     res = sum(sum_results)
        #     # print(res)
        #     result.append(res)
        #
        # # print("Before Normalization: ", result)
        #
        # # Normalization
        # for i in range(len(result)):
        #     result[i] = (result[i] - min(result)) / (max(result) - min(result))
        #
        # # print("After Normalization: ", result)
        #
        # n_total = 0
        # for i in range(0, len(local_weights)):
        #     # print("client {} dataset {}".format(idxs_users[i], local_weights[i][0]))
        #     result[i] = result[i] * local_weights[i][0]
        #     n_total += local_weights[i][0]
        # f = sum(result) / n_total
        #
        ...
        #     bi_loss_grad = f
        #     # print(bi_loss_grad)
        #     self.lambdas[idx] = abs((self.lambdas[idx] - lr * bi_loss_grad).to(self.device))
        #     print("After: client {} | lambda {}".format(idx, self.lambdas[idx]))
        #     print()
    ...
    def train(self):
        if self.update_lambda==1:
            result_path = os.path.join(self.result_dir, "update_lambda", "regular" if self.regular else "unregular")
            log_path = os.path.join(self.result_dir, "update_lambda", "log", "log.txt")
        else:
            log_path = os.path.join(self.result_dir, "non_update_lambda", "log", "log.txt")
            result_path = os.path.join(self.result_dir, "non_update_lambda")
        ...
        for epoch in range(self.args.epochs):
            ...
            if self.args.control:
                ...
                local_losses, local_grads = self.client_train(idxs_users, global_model, user_groups, epoch, train_dataset, train_losses, local_weights, local_losses, local_grads, local_delta)
            ...
            if self.update_lambda==1:
                ...
                if self.regular==1:
                    ...
                    self.lambda_update_noRegular(lr=self.lr_lambda, local_weights=local_weights, idxs_users=idxs_users, local_grads=local_grads)
            ...
            if (epoch + 11) % 10 == 0 or epoch == self.args.epochs - 1:
                if self.update_lambda==1:
                    save_result(self.args, os.path.join(result_path, str(self.args.lr_lambda), "_train_loss.txt"), str(train_losses)[1:-1])
                    save_result(self.args, os.path.join(result_path, str(self.args.lr_lambda), "_test_accuracy.txt"), str(test_accuracys)[1:-1])
                else:
                    save_result(self.args, os.path.join(result_path, "_train_loss.txt"),
                                str(train_losses)[1:-1])
                    save_result(self.args, os.path.join(result_path, "_test_accuracy.txt"),
                                str(test_accuracys)[1:-1])

            print("epoch{:4d} - loss: {:.4f} - accuracy: {:.4f} - lr: {:.4f} - time: {:.2f}".format(epoch, test_loss, test_acc, self.args.lr, time.time() - start))
            print()
if __name__ == '__main__':
    args = args_parser()
    t = FedHA(args, iid=False, equal=True, unequal=False, unbalance=False)
    t.train()
