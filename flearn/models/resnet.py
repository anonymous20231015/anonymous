...
class ResNet2(nn.Module):
    def __init__(self, ResidualBlock, in_channel=3, num_classes=10, config=None):
        ...
        self.features = nn.Sequential(
            ...
            nn.ReLU(),
            self.make_layer(ResidualBlock, [config[0], config[1], config[2], config[3], config[4]], 2,
                                          stride=1),
            self.make_layer(ResidualBlock, [config[4], config[5], config[6], config[7], config[8]], 2,
                                          stride=2),
            self.make_layer(ResidualBlock, [config[8], config[9], config[10], config[11], config[12]], 2,
                                          stride=2),
            self.make_layer(ResidualBlock, [config[12], config[13], config[14], config[15], config[16]], 2,
                                      stride=2),
        )
        self.classifier = nn.Sequential(nn.Linear(config[16], num_classes))
...
class ResNet_with_head(nn.Module):
    def __init__(self, ResidualBlock, in_channel=3, num_classes=10, config=None):
        super(ResNet_with_head, self).__init__()
        if config is None:
            config = [64, 64, 64, 64, 64, 128, 128, 128, 128, 256, 256, 256, 256, 512, 512, 512, 512]
        self.inchannel = config[0]
        self.features = nn.Sequential(
        ...
        self.classifier = nn.Sequential(nn.Linear(config[16], 84),
                                        nn.Linear(84, 84),
                                        nn.Linear(84, 256),
...
def ResNet_head(in_channel=3, num_classes=10, config=None):
    return ResNet_with_head(ResidualBlock, in_channel=in_channel, num_classes=num_classes, config=config)
...
if __name__ == "__main__":
    ...
    # model = resnet18(in_channel=3, num_classes=200)
    # print(model)
    ...
    # model.to("cpu")
    #
    model = ResNetOrigin(in_channel=3, num_classes=200)
    # print(model)
